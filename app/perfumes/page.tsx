  ...
  return (
    <>
      <section className="relative h-[32vh] min-h-[260px] sm:h-[38vh] sm:min-h-[300px] lg:h-[50vh] lg:min-h-[380px] w-full">
        {heroImage && <SafeImage src={heroImage} alt="Luxury perfumes" fill className="object-cover" priority sizes="100vw" />}
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-black/40" />
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center max-w-xl px-6">
            <span className="hidden sm:inline-block text-[10px] tracking-[0.35em] uppercase text-gold-light font-body mb-5">Maison Dunia — Fragrance</span>
            <h1 className="font-serif text-2xl sm:text-5xl lg:text-6xl font-medium text-ivory leading-tight">Luxury Perfumes</h1>
            <div className="hidden sm:block mx-auto mt-5 h-px w-12 bg-gold/50" />
            <p className="hidden sm:block mt-5 max-w-md mx-auto font-serif text-base leading-relaxed text-ivory/70 italic">Exquisite fragrances crafted with rare ingredients from around the world.</p>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-6 py-20 lg:px-12 lg:py-28">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="relative aspect-[4/5] overflow-hidden">
              {storyImage && <SafeImage src={storyImage} alt="Perfume craftsmanship" fill className="object-cover" sizes="(max-width:1024px) 100vw,50vw" />}
