...
export default function VipAbayasPage() {
    ...
    (async () => {
      try {
        const [prods, hero, story] = await Promise.all([
          getProductsByCategory("VIP Abayas"),
          getImage("hero_vip"),
          getImage("story_vip"),
        ]);
        setProducts(prods);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="max-w-lg">
              ...
              <h2 className="font-serif text-3xl sm:text-4xl font-medium text-ivory leading-tight">
                The Pinnacle of
                <br />
                <span className="italic font-normal text-gold/80">Modest Luxury</span>
              </h2>
              <div className="my-6 h-px w-12 bg-gold/40" />
              <p className="font-serif text-base text-ivory/65 leading-relaxed">
                Our VIP collection represents the zenith of artisanal
                craftsmanship. Each abaya features hand-sewn Swarovski crystals,
                24-karat gold thread embroidery, and fabrics sourced from the
                world&rsquo;s most prestigious mills. Limited to twelve pieces
                per design.
              </p>
              <div className="mt-8 flex flex-wrap gap-8">
                {[
                  { label: "Crystal Detailing", value: "Swarovski" },
                  { label: "Gold Thread", value: "24K" },
                  { label: "Limited Per Design", value: "12" },
                ].map((s) => (
                  <div key={s.label}>
